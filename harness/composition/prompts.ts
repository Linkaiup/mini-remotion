...
# 可用 API(来自 "../core")
useCurrentFrame, interpolate, spring, Easing, random, Sequence, Img, Video, OffthreadVideo, staticFile, Audio
...
# 视频素材
- 导出质量优先:用 \`<OffthreadVideo src={staticFile("xxx.mp4")} />\`（Node FFmpeg 抽帧,不依赖浏览器 decode）
- 简单场景可用 \`<Video />\`（导出时浏览器内 seek + 截图,较慢）
- 预览时两者均表现为可播放视频。
